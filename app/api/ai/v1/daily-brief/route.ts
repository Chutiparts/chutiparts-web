...
// get_daily_brief — SAFE SUMMARY (Phase 1). ไม่ refactor logic Daily Brief เดิม.
// รวม leads ค้าง + สต็อกหมด + งานค้าง จากข้อมูลที่มี · ข้อมูลไม่พอ = fallback item (ไม่ให้ล้ม) · customer contact masked
...
export async function GET(req: NextRequest) {
  ...
  const top = Math.min(Math.max(Number(searchParams.get('top') || 5) || 5, 1), 20)
  ...
  // leads ค้าง (open) — score by aging + no owner
  ...
  sheetStock
    ...
    .forEach((x) => {
      items.push({
        type: 'stock',
        title: `สต็อกหมด: ${x.name}`,
        detail: `${x.model} · SKU ${x.sku}`,
        action: 'หาเพิ่ม/ถ่ายรูป',
        owner: null,
        score: 200,
      })
    })

  // งานค้าง (open tasks) — overdue ได้คะแนนเพิ่ม
  tasks.filter(taskOpen).forEach((t: any) => {
    const overdue = t.due_date ? daysSince(t.due_date) : 0
    items.push({
      type: 'task',
      title: t.title || '(งานไม่มีชื่อ)',
      detail: `${t.task_type || 'งาน'} · ${norm(t.status) || 'todo'}`,
      action: 'ทำ/อัปเดตงาน',
      owner: t.owner || null,
      score: 120 + Math.max(overdue, 0),
    })
  })
  ...
  await audit(supa, auth.tid, 'get_daily_brief', { top }, brief.length)
