...
// SAFETY: dry-run เสมอ · ไม่เขียนจนกดยืนยัน · ห้ามทับด้วยว่าง (จัดการฝั่ง server) · เตือน SKU ซ้ำ · backup ก่อนเขียน · import report
...
// หา index คอลัมน์จากชื่อหัว (ยืดหยุ่น)
...
type PreviewRow = { status: 'new' | 'update' | 'nochange' | 'warning' | 'error'; sku: string; name: string; car_model: string; price: number | null; oem_number: string; note: string; id?: string | null; write: boolean }

export default function SyncClient({ products, applyProductSync }: { products: Row[]; applyProductSync: (rows: SyncRow[]) => Promise<{ ok: boolean; added: number; updated: number; errors: { sku: string; msg: string }[]; batchId: string; at: string }> }) {
  ...
  const bySku = useMemo(() => { const m: Record<string, Row> = {}; products.forEach((p) => { const k = normKey(p.sku); if (k) m[k] = p }); return m }, [products])
  ...
  const analysis = useMemo(() => {
    ...
    if (ci.sku < 0) return { error: 'ไม่พบคอลัมน์ SKU ใน CSV (ตรวจว่า export ถูกแท็บ)', rows: [] as PreviewRow[], counts: {} as any, seenDup: [] as string[], orphans: 0, headerRow: hi + 1 }
    ...
    for (let i = hi + 1; i < grid.length; i++) {
      ...
      // dedupe: SKU ซ้ำ (หลายรับเข้า) → เก็บครั้งแรก + เตือน
      if (usedSku.has(key)) continue
      usedSku.add(key)
      const ex = bySku[key]
      let status: PreviewRow['status'], note = ''
      if (!name && !price && !oem && !model) { status = 'error'; note = 'แถวข้อมูลว่าง' }
      else if (!ex) { status = 'new'; note = 'SKU ใหม่ (จะตั้งซ่อนไว้ก่อน is_published=false)'; if (!name) note = 'SKU ใหม่ แต่ไม่มีชื่อ' }
      else {
        const diffs: string[] = []
      ...
      out.push({ status, sku, name, car_model: model, price, oem_number: oem, note, id: ex ? ex.id : null, write: status === 'new' || status === 'update' })
    }
    const seenDup = Object.keys(seen).filter((k) => seen[k] > 1)
    const sheetKeys = new Set(out.map((o) => normKey(o.sku)))
    const orphans = products.filter((p) => normKey(p.sku) && !sheetKeys.has(normKey(p.sku))).length
    const counts = {
      ...
      warning: out.filter((o) => o.status === 'warning').length,
      error: out.filter((o) => o.status === 'error').length,
    }
    return { error: '', rows: out, counts, seenDup, orphans, headerRow: hi + 1 }
  }, [raw, bySku, products])
  ...
  async function confirmWrite() {
    ...
    setBusy(true)
    backup() // SAFETY#6: snapshot ก่อนเขียน
    try {
      const payload: SyncRow[] = toWrite.map((r) => ({ id: r.id, sku: r.sku, name: r.name || null, car_model: r.car_model || null, price: r.price, oem_number: r.oem_number || null }))
      const res = await applyProductSync(payload)
      setResult(res)
      // SAFETY#5: import report
  ...
  return (
    <div style={{ minHeight: '100vh', background: CREAM, fontFamily: '-apple-system,"Segoe UI","Noto Sans Thai",sans-serif', color: '#1a1a1a' }}>
      <div style={{ background: GREEN, color: '#fff', padding: '14px 16px' }}>
        ...
        <div style={{ fontSize: 11.5, color: '#a9bfb1', marginTop: 4 }}>ปลอดภัย: dry-run เสมอ · ไม่ทับด้วยค่าว่าง · SKU ใหม่ซ่อนไว้ก่อน · backup อัตโนมัติก่อนเขียน · เฉพาะ owner</div>
      </div>
      ...
      <div style={{ padding: 12, maxWidth: 1000, margin: '0 auto' }}>
        ...
        {analysis && !analysis.error && (
          <>
            <div style={card}>
              ...
              {analysis.seenDup.length > 0 && <div style={{ fontSize: 12.5, color: '#92400E', marginBottom: 4 }}>⚠️ SKU ซ้ำใน CSV {analysis.seenDup.length} ตัว (หลายรับเข้า) — ใช้แถวแรกทำแคตตาล็อก: {analysis.seenDup.slice(0, 8).join(', ')}{analysis.seenDup.length > 8 ? ' …' : ''}</div>}
              {analysis.orphans > 0 && <div style={{ fontSize: 12.5, color: '#5F5E5A' }}>ℹ️ มีในเว็บแต่ไม่มีในชีต {analysis.orphans} SKU — <b>ไม่ลบ</b> (ปล่อยไว้ให้พี่ตรวจเอง)</div>}
            </div>

            <div style={{ ...card, padding: 0, overflow: 'auto' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12.5 }}>
                <thead><tr style={{ background: '#f3f1ea', textAlign: 'left' }}>
                  {['สถานะ', 'SKU', 'ชื่อ', 'รุ่น', 'ราคา', 'OEM', 'หมายเหตุ'].map((h) => <th key={h} style={{ padding: '8px 10px', borderBottom: '1px solid #e7e3d8', whiteSpace: 'nowrap' }}>{h}</th>)}
                </tr></thead>
                <tbody>
            ...
            <div style={{ ...card, background: '#fbfaf6' }}>
              ...
              <div style={{ fontSize: 11.5, color: '#999', marginTop: 6 }}>กดยืนยัน = ดาวน์โหลด backup ให้อัตโนมัติ แล้วเขียน · SKU ใหม่ตั้ง is_published=false (ต้องไปเปิด publish + ใส่รูปเองทีหลัง)</div>
            </div>
          </>
        )}
        ...
        {result && (
          <div style={{ ...card, background: result.errors.length ? '#FCEBEB' : '#EAF3DE' }}>
            ...
            <div style={{ fontSize: 11.5, color: '#777', marginTop: 6 }}>ดาวน์โหลด import report + backup ไว้แล้ว (ในโฟลเดอร์ดาวน์โหลด) · SKU ใหม่ไปเปิด publish ที่หน้าจัดการสินค้า</div>
          </div>
        )}
      </div>
    </div>
