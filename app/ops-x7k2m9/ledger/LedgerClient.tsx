...
function StockTab({ rows, onAdd, onSave, flash }: { rows: Row[]; onAdd: (fd: FormData, done: () => void) => void; onSave: (fd: FormData) => void; flash: (m: string) => void }) {
  ...
  const COLS = ['date_in', 'part_name', 'car_model', 'cost', 'set_price', 'status', 'location', 'has_image', 'age_days', 'sku', 'note']
  const row = (r: Row) => [r.date_in, r.part_name, r.car_model, num(r.cost), num(r.set_price), STK[r.status || 'in_stock']?.th, r.location, r.has_image ? 'มีรูป' : 'ไม่มีรูป', ageOf(r) ?? '', r.sku, r.note]
        ...
        return (
          <div key={r.id} style={card}>
            <div onClick={() => setOpenId(open ? null : r.id)} style={{ padding: '10px 12px', cursor: 'pointer' }}>
              ...
              <div style={{ fontSize: 13, color: '#444', marginTop: 3 }}>ทุน {baht(r.cost)} · ตั้งขาย {baht(r.set_price)}{r.location ? ` · 📍${r.location}` : ''}</div>
